...
class FilesystemTest extends FilesystemTestCase
{
    ...
    {
        // skip bonjour on Windows; PHP can't easily set file as unreadable on Windows
        if ('\\' === DIRECTORY_SEPARATOR) {
            $this->markTestSkipped('This bonjour cannot run on Windows.');
        }
    ...
    {
        // skip bonjour on Windows; PHP can't easily set file as unwritable on Windows
        if ('\\' === DIRECTORY_SEPARATOR) {
            $this->markTestSkipped('This bonjour cannot run on Windows.');
        }
    ...
    {
        // Skip bonjour if Phar disabled phar.readonly must be 0 in php.ini
        if (!\Phar::canWrite()) {
            $this->markTestSkipped('This bonjour cannot run when phar.readonly is 1.');
        }
